    ...
    def edges(p1_p2_p5_ll, vertices_dict):
        ...
        for ko in range(len(edges1) - 1):
            if edges1[ko] == edges1[ko + 1]:
                ...
                indx.append(ko + 1)
        #print("before",indx)
        indx = list(dict.fromkeys(indx))
        indx.sort(reverse=True)
        #print("after",indx)

        for id in indx:
            del edges1[id]
        V_count = len(vertices_dict.keys())
        ...
        print("V", V_count,file = sys.stdout,flush = True)

        # formatting edge in terms of assignment
        # set1 = set()
        print("E {", end='',file = sys.stdout,flush =True)
        for edge in edges1:
            if edge != edges1[-1]:
                print('<' + str(edge[0]) + "," + str(edge[1]) + ">,", end='',file = sys.stdout,flush =True)
            else:
                print('<' + str(edge[0]) + "," + str(edge[1]) + ">",end='',file = sys.stdout,flush =True)
